...
class ProfileRepositoryImpl @Inject constructor(
    ...
    override suspend fun getClients(): List<ClientCardEntity> =
        withContext(CoroutineScope(Dispatchers.IO).coroutineContext) {
            val clients = getClientsFor(getOwnPsychologistId())

            clients.pmap {
                val ref = Firebase.database(AppModule.URL).reference.child(it.value)
                ClientCardEntity(it.value, getUserName(ref), getRequest(ref).joinToString())
            }
        }

    private suspend fun getClientsFor(psychologistId: String): HashMap<String, String> =
        suspendCoroutine { continuation ->
            Firebase.database(AppModule.URL).reference.child(psychologistId)
                .child(PsychologistRepositoryImpl.CLIENTS).get()
                .addOnSuccessListener { snapshot ->
                    continuation.resume(
                        snapshot.getTypedValue<HashMap<String, String>>() ?: hashMapOf()
                    )
                }
                .addOnFailureListener {
                    continuation.resumeWithException(it)
